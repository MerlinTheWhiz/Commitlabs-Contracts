...
impl CommitmentInterface {
    ...
    ///
    /// # Arguments
    /// * `owner` - Address to query commitments for
    ///
    /// # Returns
    /// * `Result<Vec<String>, Error>` - Vector of commitment IDs
    ///
    /// # Errors
    /// Returns empty Vec if owner has no commitments (not an error condition).
    ///
    /// # Security
    /// This is a read-only view function. No authorization required.
