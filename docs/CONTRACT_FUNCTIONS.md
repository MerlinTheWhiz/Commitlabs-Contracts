...
| get_admin() -> Result<Address>                                                | Fetch admin address.              | View.                  | Fails if not initialized.                                                                         |
| get_core_contract() -> Result<Address>                                        | Fetch core contract address.      | View.                  | Fails if not initialized.                                                                         |
| get_stored_health_metrics(commitment_id) -> Option<HealthMetrics>             | Fetch cached health metrics.      | View.                  | Returns None until an attestation updates the cache; read-only metric queries do not populate it. |
